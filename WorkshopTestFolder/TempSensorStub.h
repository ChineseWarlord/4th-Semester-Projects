...
void* TempGen(){ //Pointer til void som return type
    int Count = 10; //Antal målinger
    float Avg[Count], sum, n;
    float avg;

    srand((unsigned int)time(NULL)); //Genererer pseudo-random numre
    ...
    float *result = (float *)malloc(sizeof(result)); //Laver en pointer 'result' og allokerer en blok af hukommelse. Den returnerer en pointer til den allokerede hukommelse "float" - svarer til 400 bytes
