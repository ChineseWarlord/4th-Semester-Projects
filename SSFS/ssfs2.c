...
// The length of the filename, directory or file content can only be 256 chars.
// -1 is an indicator of the starting index of the arrays.
...
static int do_read( const char *path, char *buffer, size_t size, off_t offset, struct fuse_file_info *fi )
{
	...
	if(offset + size > len){
		memcpy(buffer, files_content + offset, len - offset);
		return len - offset;
	}

	memcpy(buffer, files_content + offset, size);
	return size;
	}
	return -ENOENT;
}
